...
/** @addtogroup PT32x066x_StdPeriph_Driver
  * @{
  */
...
void CLOCK_Cmd(void)
{
	#if (HSE_ENABLE == 1)
		#if (HSE_SRC== 0)
			GPIO_AnalogRemapConfig(AFIOB, GPIO_Pin_8, ENABLE);
			GPIO_AnalogRemapConfig(AFIOB, GPIO_Pin_9, ENABLE);
	...
	//	RCC_ClockSourceConfig(RCC_ClockSource_HSI, DISABLE);	//关闭内部RC
		...
		#else
			GPIO_AnalogRemapConfig(AFIOB, GPIO_Pin_10, ENABLE);
			GPIO_AnalogRemapConfig(AFIOB, GPIO_Pin_11, ENABLE);
	...
	#if (LSI_FREQS == 0)
		RCC_LSIFreqConfig(RCC_LSIFreq_38400Hz);
	#else
		RCC_LSIFreqConfig(RCC_LSIFreq_32768Hz);
	#endif
}
...
void CLOCK_SystemClockConfig(void)
{
		...
		#else
			Wait_ClockReady();
			RCC_SetSystemClock(RCC_SYSCLK_HSE);
			//时钟安全机制处理
			if(HSI_ENABLE == 0)
			{
				RCC_ClockSourceConfig(RCC_ClockSource_HSI, DISABLE);	//关闭内部RC 32M
			}
		#endif
	#endif
...
void CLOCK_MCOConfig(void)
{
		...
		RCC_MCOSetPrescaler(MCO_DIV<<4);
			...
			GPIO_AnalogRemapConfig(AFIOC, GPIO_Pin_13, DISABLE);
			GPIO_DigitalRemapConfig(AFIOC, GPIO_Pin_13, AFIO_AF_1, ENABLE);
		#elif (MCO_PINSEL == 1)
			RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOC, ENABLE);
			GPIO_AnalogRemapConfig(AFIOC, GPIO_Pin_15, DISABLE);
